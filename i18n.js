...
(function () {
  if (window.__LI_I18N_BOOTED) return;   // never run twice, even if loaded twice
  window.__LI_I18N_BOOTED = true;
  var LANGS = {
  ...
  function walk() {
    ...
    while ((n = tw.nextNode())) {
      ...
      var want;
      if (tr === undefined || tr === null) { want = n.__liEn; }
      else {
        // keep the original node's surrounding whitespace so inline text doesn't fuse
        var ws = n.__liEn.match(/^(\s*)[\s\S]*?(\s*)$/);
        want = ws[1] + tr + ws[2];
      }
  ...
  function buildSwitcher(nav) {
    ...
    wrap.setAttribute('data-li-injected', '1');
  ...
  var tries = 0, navSeen = 0;
  function pageHasNativeSwitcher() {
    // the page's own switcher is marked with a native marker attr in its template
    // source; if the (possibly still-streaming) template declares one, never inject
    var sc = document.scripts;
    for (var i = 0; i < sc.length; i++) {
      if (!sc[i].src && sc[i].textContent && sc[i].textContent.indexOf('data-li-' + 'native') !== -1) return true;
    }
    return false;
  }
  var injector = setInterval(function () {
    tries++;
    if (pageHasNativeSwitcher() || document.querySelector('button[aria-label="Change language"]')) { clearInterval(injector); return; }
    var nav = document.querySelector('header nav');
    // wait a few extra ticks after the nav appears: a page's own switcher may
    // still be streaming in, and injecting early would duplicate it
    if (nav && ++navSeen >= 4) { clearInterval(injector); buildSwitcher(nav); }
    else if (tries > 40) clearInterval(injector);
  }, 300);

  /* safety sweep: if a native switcher appears later (or anything ever
     duplicates), remove injected copies so exactly one switcher remains */
  setInterval(function () {
    var all = document.querySelectorAll('button[aria-label="Change language"]');
    if (all.length <= 1) return;
    var native = false;
    for (var i = 0; i < all.length; i++) if (!all[i].closest('[data-li-injected]')) native = true;
    var keptInjected = false;
    for (var j = 0; j < all.length; j++) {
      var w = all[j].closest('[data-li-injected]');
      if (!w) continue;
      if (native || keptInjected) { w.parentNode && w.parentNode.removeChild(w); if (w === wrap) { wrap = null; btn = null; menu = null; } }
      else keptInjected = true;
    }
  }, 700);
